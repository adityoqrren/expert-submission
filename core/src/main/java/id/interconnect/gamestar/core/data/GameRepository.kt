...
class GameRepository(
    ...
    override fun getAllGames(): Flow<Resource<List<Game>>> =
        object :
            id.interconnect.gamestar.core.data.NetworkBoundResource<List<Game>, List<GameItemResponse>>() {
    ...
    override fun getDetailGame(id: Int): Flow<Resource<Game>> {
        return object :
            id.interconnect.gamestar.core.data.NetworkBoundResource<Game, GameResponse>() {
            override fun loadFromDB(): Flow<Game> {
                return localDataSource.getDetailGame(id)
